...
async function getBaleDefaultPrices(baleId: Types.ObjectId): Promise<{
  ...
  const bale = await BaleModel.findById(baleId)
    .select("pricePerTon pricePerUnit")
    .lean();
  if (!bale) {
    throw new Error("סוג חבילה לא נמצא");
  }
...
async function resolveCustomerObjectId(
  customerId: unknown,
...
async function buildTrackingPatch(
  ...
  if (mustHave("transportPrice")) {
    patch.transportPrice = parseOptionalPositiveNumber(
      body.transportPrice,
      "מחיר הובלה",
    );
  }
...
export const baleOrderTrackingService = {
  ...
  async create(body: Record<string, unknown>): Promise<ApiDocument> {
    ...
    const prices = await resolveStoredPrices(body, patch.bale, {
      requireAll: false,
    });
    ...
    const populated = await baleOrderTrackingRepository.findById(
      String(created._id),
    );
  ...
  async update(
    ...
    const pricingForm =
      patch.pricingForm ??
      inferPricingFormFromDoc(
        existing as { pricingForm?: unknown; weight?: unknown },
      );
